...
"""配置文件加载模块"""
# 输入:配置文件路径,输出:包含配置项的字典。配置项优先级: mykey.py > mykey.json > 环境变量。
def _load_mykeys():
    global _mykey_path
    try:
        # importlib: Python 标准库中的导入系统工具模块，可进行动态导入、重载模块等操作
        # reload(mykey): 重新执行模块代码，确保读取到 mykey 文件的最新修改内容（而非旧缓存）
...
# 赋值全局变量为none，触发第一次访问时加载mykeys并更新全局变量。后续访问时检查文件修改时间，自动重新加载。
_mykey_path = _mykey_mtime = None
def reload_mykeys():
    global _mykey_mtime
    mt = os.stat(_mykey_path).st_mtime_ns if _mykey_path else -1
    # 如果文件修改时间没有变化，直接返回当前 mykeys，避免频繁访问磁盘。注意 mykeys 可能尚未加载，此时返回空 dict。
...
def trim_messages_history(history, context_win):
    ...
    compress_history_tags(history)  # L1
...
class ToolClient:
    ...
    def chat(self, messages, tools=None):
        ...
        _write_llm_log('Response', raw_text)# 写入的是文本段,不做dict解析;
        # 返回的时候,将thinking,tool_use作为单独字段,并在content中去除;
...
"""
    ...
    def _parse_mixed_response(self, text):
        ...
        # 移除thinking块,并提取其中内容作为thinking,剩余文本继续解析工具调用
        if think_match:
            thinking = think_match.group(1).strip()
            remaining_text = re.sub(r"<think(?:ing)?>(.*?)</think(?:ing)?>", "", remaining_text, flags=re.DOTALL)
        # 首先尝试解析标准的tool_use块,如果没有,再尝试从文本中提取工具调用信息
        # 如果有,将工具调用信息从文本中移除,剩余文本作为最终内容返回
