...
def _load_l4_batch_process():
    """Load L4 compressor by file path to avoid sys.path-dependent imports."""
    spec = importlib.util.spec_from_file_location('ga_l4_compress_session', _L4_COMPRESSOR)
    if spec is None or spec.loader is None:
        raise ImportError(f'Cannot load L4 compressor: {_L4_COMPRESSOR}')
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.batch_process
