...
| 待收敛项 | 旧测试引用 | 待收敛 | `tests/test_llm_client.py` 仍引用已删除的 `core.llm_client`。 |
| 待收敛项 | 多渠道入口 | 非当前主目标 | 暂不把聊天平台、Web 服务和多端协议作为核心闭环。 |
| 待收敛项 | 复杂浏览器控制 | 非主路径 | `web_tools` 相关导入仍处于注释状态。 |
| 待收敛项 | 本地状态目录管理 | 待规范 | `temp/`、`sche_tasks/`、`memory/` 带有运行状态，需要按环境管理。 |

## 后续计划

- 收敛依赖声明，补充最小可复现安装路径。
- 统一 `llmcore.py` 与 `core/` 重构模块的职责边界。
- 清理或修复旧测试，让 discover 路径恢复可信。
